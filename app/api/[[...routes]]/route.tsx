...
const getDetailsFromInitialPath = (initialPath: string) => {
  ...
  const networkId = vars[3];
  const roundId = vars[4];
  const projectId = vars[5];
...
//         const { bannerImage } = await queryGitcoinProject(
//           networkId,
//           roundId,
//           projectId
//         );
...
//           {c.var.bannerImage && (
//             <img
//               src={`https://ipfs.io/ipfs/${c.var.bannerImage}`}
//               style={{ width: "100%" }}
//             />
//           )}
//         </div>
...
//       intents: [
//         <Button.Redirect
//           location={`https://explorer.gitcoin.co/#/${urlSearch}`}
//         >
//           Donate
//         </Button.Redirect>,
...
app.frame(
  ...
  (c) => {
    ...
    return c.res({
      ...
      intents: [
        <Button action="/select-asset">Donate</Button>,
        <Button action="/what-is-qf">What is QF?</Button>,
      ],
    });
  }
);
...
app.frame("/confirm-donate", async (c) => {
  ...
  return c.res({
    ...
    intents: [
      <Button action="/sign-message">Continue</Button>,
      <Button.Reset>Back</Button.Reset>,
    ],
  });
});
...
app.frame("/sign-message", async (c) => {
  ...
  const { networkId, roundId, projectId } =
    getDetailsFromInitialPath(initialPath);
  console.log({ networkId, roundId, projectId });
  const { round, anchorAddress } = await queryGitcoinProject(
    networkId,
    roundId,
    projectId
  );
  ...
  return c.res({
    ...
    intents: [
      ...
      <Button.Redirect location={`/sign/${messageHash}`}>
        Sign Message
      </Button.Redirect>,
      <Button.Reset>Back</Button.Reset>,
    ],
...
app.transaction("/donate", async (c) => {
  ...
  const preparedTx = await contractOrigin.callDepositV3.populateTransaction(
    depositParameters,
    messageCombined
  );
...
app.frame("/what-is-qf", (c) => {
  ...
  const nextButton =
    state.qfSlide < 2 ? [<Button value="next">NEXT</Button>] : [];
  const prevButton =
    state.qfSlide > 0 ? [<Button value="prev">PREVIOUS</Button>] : [];
      ...
      case 1:
        return (
          ...
          >
            <h1>What is QF?</h1>
            <p>
              Quadratic Funding (QF) is a crowdfunding approach that leverages
              community donations to allocate funds. Donations act as votes,
              with broader support resulting in larger matches.
            </p>
          </div>
        );
      default:
        return (
          ...
          >
            ...
            <h3 style={{ margin: 0, marginTop: 15 }}>
